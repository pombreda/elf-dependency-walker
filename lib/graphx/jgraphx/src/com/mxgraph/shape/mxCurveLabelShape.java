/**
 * $Id: mxCurveLabelShape.java,v 1.21 2010-12-15 14:35:03 david Exp $
...
public class mxCurveLabelShape implements mxITextShape
{
						...
						rect)))
		{
			...
			for (int j = 0; j < text.length(); j++)
			{
				...
				if (labelGlyphs[j].visible && parallel != null && parallel != mxCurve.INVALID_POSITION)
				{
					...
					double rotation = (Math.atan(parallelEnd.getY()
							/ x));
	...
	public boolean intersectsRect(Rectangle rect)
	{
		...
		{
			if (rect.intersects(labelGlyphs[i].drawingBounds.getRectangle()))
			{
				return true;
			}
		}
